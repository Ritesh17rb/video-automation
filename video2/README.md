...
| --- | --- | --- | --- | --- |
| `frames/frame01.webp` (00:01.3) | Custom Scenario card visible before any typing. | `#domain-input`. | Static ID rendered on load. | Assumption: clearing storage always reveals this card; script focuses input immediately. |
| `frames/frame02.webp` (00:01.6) | “india” typed with the **Go** button in view. | `#start-custom-btn`. | Only CTA inside the custom scenario component. | Assumption: button label stays “Go”; automation clicks the ID’d button. |
| `frames/frame03.webp` (00:08.7) | Mission Briefing modal open. | `#prepModal`. | Bootstrap modal ID exported by the app. | Assumption: modal always appears after clicking Go; script waits for visibility. |
| `frames/frame04.webp` (00:13.7) | Hard pill highlighted inside the modal. | `label[for="diff-hard"]`, `#diff-hard`. | Radio inputs and labels share hard-coded IDs. | Assumption: Hard option exists; script toggles label and asserts checked state. |
| `frames/frame05.webp` (00:13.8) | Launch button about to start the simulation. | `#btn-launch-sim`. | Unique ID in the modal footer. | Assumption: clicking once triggers the async mount; script pauses to match timing. |
| `frames/frame06.webp` (00:21.4) | Board rendered with `ROLL DICE` button in focus. | `#roll-btn`. | Button created after the game scene loads. | Assumption: ID stays unique; script waits up to 60s before clicking. |
| `frames/frame07.webp` (00:21.4) | Game log reflecting the die landing. | `#game-log`. | Log text is the easiest signal that the roll finished. | Assumption: log text updates per roll; helper waits before checking overlay. |
| `frames/frame08.webp` (00:21.4) | Question overlay acquires the `active` class. | `#question-modal-overlay`. | Overlay remains in DOM; `classList` reveals visibility. | Assumption: `active` class is authoritative; script polls `.includes('active')`. |
| `frames/frame09.webp` (00:25.2) | Option buttons rendered. | `#modal-options button`. | Buttons share the same selector each run. | Assumption: at least one option exists; script prefers the second button when present. |
| `frames/frame10.webp` (00:26.2) | Feedback block visible after answering. | `#modal-feedback`. | ID is stable even when the overlay is transparent. | Assumption: answering any option triggers feedback; script asserts visibility before closing. |
| `frames/frame11.webp` (00:27.5) | Overlay closed, board visible again. | `#modal-close-btn`, overlay class. | Close button plus a class check confirm dismissal. | Assumption: close button always present; script clicks it and polls for class removal. |
